...
const PhotosUploader: React.FC<PhotosUploaderProps> = ({
  ...
  function removePhoto(link: string) {
    onChange([...addedPhotos.filter((photo) => photo !== link)]);
  }

  function addAsMainPhoto(link: string, event: any) {
    event.preventDefault();
    const addedPhotosWithoutSelected = addedPhotos.filter(
      (photo) => photo !== link
    );
    const newAddedPhoto = [link, ...addedPhotosWithoutSelected];
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4 ">
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div key={link} className="relative  border-red-500 object-cover">
              <button
                onClick={() => removePhoto(link)}
                className="absolute -right-1 -top-1 z-10 box-border cursor-pointer rounded-full border-gray-900 bg-red-600 p-2 text-slate-50 transition-all hover:scale-110 "
              >
                <svg
              ...
              <button
                onClick={(event) => addAsMainPhoto(link, event)}
                className="absolute bottom-1 left-1 z-10 box-border cursor-pointer rounded-full bg-opacity-0 bg-none p-2 text-slate-50 transition-all hover:scale-110 "
              >
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <img
                ...
                alt=""
                className="h-[150px] min-h-full w-full rounded-2xl object-cover"
              />
            </div>
          ))}
